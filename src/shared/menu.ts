...
export function makeMenuIcon(
    ...
    button.className = `s-editor-btn flex--cell js-editor-btn js-${key}`;
...
export function dropdownItem(
    ...
    const button = document.createElement("button");
    button.type = "button";
    button.className = `s-btn s-btn__unset flex--cell ta-left px12 py4 h:bg-black-050 c-pointer js-editor-btn`;
    button.dataset.key = key;
...
export function dropdownSection(title: string, key: string): MenuCommandEntry {
    ...
    section.className = `flex--cell ta-left fs-fine tt-uppercase mx12 mb6 mt12 fc-black-400`;
...
export function makeMenuLinkEntry(
    ...
    dom.className = `s-editor-btn js-editor-btn flex--cell`;
    dom.href = href;
    dom.target = "_blank";
    dom.title = title;
