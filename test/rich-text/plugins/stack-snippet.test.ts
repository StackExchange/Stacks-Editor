import {
    openSnippetModal
} from "../../../src/shared/plugins/stack-snippets/commands";
import {
    getSnippetMetadata, mapMetaLine, MetaLine,
    SnippetMetadata,
    StackSnippetOptions, validateMetaLines
} from "../../../src/shared/plugins/stack-snippets/common";
import {EditorState} from "prosemirror-state";
import {
    cleanupPasteSupport,
    createState, createView, dispatchPasteEvent,
    setupPasteSupport,
    testRichTextSchema
} from "../test-helpers";
import {
...
import {
    parseSnippetBlockForProsemirror, stackSnippetPasteHandler
} from "../../../src/shared/plugins/stack-snippets/paste-handler";
import {EditorView} from "prosemirror-view";
import {Node as ProseMirrorNode} from "prosemirror-model";
import {
    StackSnippetView
} from "../../../src/shared/plugins/stack-snippets/snippet-view";

describe("Stack-snippet", () => {
    describe("commands", () => {
        const whenOpenSnippetCommandCalled =
            (state:EditorState, shouldMatchCall: (meta?: SnippetMetadata, js?: string, css?: string, html?: string) => boolean) => {
        ...
        it('should do nothing if dispatch null', () => {
            ...
            const state = createState("Here's a paragraph -  a text block mind you", []);
        ...
        it.each(validSnippetRenderCases)("should send openModal with blank arguments if snippet detected", (markdown: string, langs: string[]) => {
            //Create a blank doc, then replace the contents (a paragraph node) with the parsed markdown.
            let state = EditorState.create({
                schema: testRichTextSchema,
                plugins: [],
            });
            state = state.apply(state.tr.replaceRangeWith(0, state.doc.nodeSize - 2, parseSnippetBlockForProsemirror(testRichTextSchema, markdown)));
    ...
    describe("common functions", () => {
        describe("getSnippetMetadata", () => {
            ...
            it("should parse begin correctly", () => {
                const validSnippetRaw = configureBegin(true, false, null, true, false) + validJs + validEnd;
                const validSnippet = parseSnippetBlockForProsemirror(testRichTextSchema, validSnippetRaw);
            ...
            it("should use an available ID if provided", () => {
                ...
                const langNode = testRichTextSchema.nodes.stack_snippet_lang.createChecked(
                    {language: "js"},
                    testRichTextSchema.text("console.log('test');")
                );
                const validSnippet = testRichTextSchema.nodes.stack_snippet.createChecked(
                    {
                        id: "1234",
                        babel: "true",
            ...
            it.each(validSnippetRenderCases)("should parse language blocks correctly", (markdown: string, langs: string[]) => {
                const validSnippet = parseSnippetBlockForProsemirror(testRichTextSchema, markdown);
        ...
        describe("validateMetaLines", () => {
            it("should validate multiple language blocks", () => {
                const metaLines: MetaLine[] = [
                    { type: "begin", index: 1, babel: "true", babelPresetTS: "true", babelPresetReact: "true", hide: "true", console: "true"},
            ...
            it("should not allow duplicate begin blocks", () => {
                const metaLines: MetaLine[] = [
                    { type: "begin", index: 1, babel: "true", babelPresetTS: "true", babelPresetReact: "true", hide: "true", console: "true"},
                    { type: "begin", index: 2, babel: "true", babelPresetTS: "true", babelPresetReact: "true", hide: "true", console: "true"},
                    { type: "lang", index: 4, language: "html"},
                    { type: "lang", index: 5, language: "html"},
                    { type: "end", index: 7 },
                ]
            ...
            it("should clip a valid selection if there are a begin and end block", () => {
                const metaLines: MetaLine[] = [
                    ...
                    { type: "lang", index: 4, language: "html"},
                    { type: "end", index: 7 },
                    { type: "begin", index: 8, babel: "true", babelPresetTS: "true", babelPresetReact: "true", hide: "true", console: "true"},
                    { type: "end", index: 9 },
                ]
            ...
            it("should validate begin comes before end blocks", () => {
                const metaLines: MetaLine[] = [
                    { type: "begin", index: 4, babel: "true", babelPresetTS: "true", babelPresetReact: "true", hide: "true", console: "true"},
    ...
    describe("paste functionality", () => {
        ...
        it.each(validSnippetRenderCases)("should render the pasted code as a stack_snippet node", (markdown: string, langs: string[]) => {
