...
import {
    ...
    createView,
    dispatchPasteEvent,
    setupPasteSupport, sleepAsync,
    testRichTextSchema,
} from "../test-helpers";
import {
    configureBegin, invalidSnippetRenderCases,
    validBegin,
    validCss,
    validEnd,
    validHtml,
...
import {
    stackSnippetPlugin
} from "../../../src/shared/plugins/stack-snippets/schema";
...
describe("Stack-snippet", () => {
    ...
    describe("stackSnippetPlugin (Markdown-it)", () => {
        ...
        it.each(validSnippetRenderCases)("should return a markdown token stream of stack_snippet tokens", (markdown: string, langs: string[]) =>{
        ...
        it.each(invalidSnippetRenderCases)("should return a token stream that does not include stack_snippet tokens", (markdown: string) =>{
            const tokens = mdit.parse(markdown, {});
            const snippetTokenTypes = ["stack_snippet_open", "stack_snippet_close", "stack_snippet_lang"];
    ...
    describe("StackSnippetView", () => {
        ...
        it("should render run code button if renderer provided", () =>{
            ...
            const runCodeButton = view.dom.querySelectorAll(".snippet-ctas > button.s-btn");

            expect(runCodeButton).toHaveLength(1);
            expect(runCodeButton[0].getAttribute("aria-label")).toBe("Run code snippet");
