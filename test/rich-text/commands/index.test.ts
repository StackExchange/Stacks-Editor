...
import {
    ...
    applySelection,
    createState,
    executeTransaction, parseHtmlToDoc,
    testRichTextSchema,
} from "../test-helpers";
...
describe("commands", () => {
    ...
    describe("escapeUnselectableCommand", () => {
        ...
        it("should do nothing if last node in the document is selected", () => {
            ...
            let state = createState("Here's a paragraph -  a text block mind you", []);
            state = state.apply(
        ...
        it("should do nothing if last inline text in the document is selected", () => {
            ...
            state = state.apply(
                state.tr.setSelection(TextSelection.create(state.doc, inlinePosEnd - 3, inlinePosEnd))
            );
        ...
        it("should not alter the document if there is a proceeding textblock node", () => {
            ...
            state = state.apply(state.tr.insert(firstNodePosEnd, parseHtmlToDoc("This is another node, wild!", false)));
            state = state.apply(state.tr.setSelection(TextSelection.create(state.doc, firstNodePosEnd - 3, firstNodePosEnd)))
        ...
        it("should add a paragraph block if there are no following textblock nodes", () => {
            let state = EditorState.create({
                doc: parseHtmlToDoc("<table><thead><tr><td>Header 1</td><td>Header 2</td></tr></thead><tbody><tr><td>one</td><td>two</td></tr></tbody></table>", false),
                schema: testRichTextSchema,
                plugins: [],
            });

            const selection = TextSelection.create(state.doc, state.doc.nodeSize - 8);
            ...
            whenEscapeUnselectableCommandCalled(state, true, (tr) => {
                ...
                if(step.slice.content.firstChild.type.name !== "paragraph") return false;
                if(step.slice.content.firstChild.textContent != "") return false;
                return true
            })
        });
    });
